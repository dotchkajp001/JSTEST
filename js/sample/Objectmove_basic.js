...
// テキストが初期値の場合消す
function deleteText( textid, istext ) {
	if(istext == "ここに入力"){
...
//オブジェクトにuserid属性追加
function AddUserZ(canvas,object){
	object.toObject = (function(toObject) {
		return function() {
			return fabric.util.object.extend(toObject.call(this), {
			userid: this.userid
...
$(document).ready(function(){
	...
	for(var i=0; i < mainMapDataLen; i++){
		...
		switch(mainMapData.DATA[i].OBJTYPE){
			case "circle":
				mainMapDataJSON += '"radius":'+mainMapData.DATA[i].RADIUS+',"scaleX":'+mainMapData.DATA[i].SCALEX+',"scaleY":'+mainMapData.DATA[i].SCALEY+',"fill":"'+mainMapData.DATA[i].FILL+'","stroke":"'+mainMapData.DATA[i].STROKE+'","strokeWidth":'+mainMapData.DATA[i].STROKEWIDTH+',"userid":"'+mainMapData.DATA[i].USERID+'"}';
			break;
			case "rect":
				mainMapDataJSON += '"height":'+mainMapData.DATA[i].HEIGHT+',"width":'+mainMapData.DATA[i].WIDTH+',"fill":"'+mainMapData.DATA[i].FILL+'","stroke":"'+mainMapData.DATA[i].STROKE+'","strokeWidth":'+mainMapData.DATA[i].STROKEWIDTH+',"userid":"'+mainMapData.DATA[i].USERID+'"}';
			break;
			case "triangle":
				mainMapDataJSON += '"height":'+mainMapData.DATA[i].HEIGHT+',"width":'+mainMapData.DATA[i].WIDTH+',"fill":"'+mainMapData.DATA[i].FILL+'","stroke":"'+mainMapData.DATA[i].STROKE+'","strokeWidth":'+mainMapData.DATA[i].STROKEWIDTH+',"userid":"'+mainMapData.DATA[i].USERID+'"}';
			break;
...
//	mainMapDataJSON += ']}'
	...
	mainMapDataJSON += '],"backgroundImage":{"type":"image","originX":"left","originY":"top","left":0,"top":0,"width":'+mainCanvasWidth+',"height":'+mainCanvasHeight+',"fill":"rgb(0,0,0)","stroke":null,"strokeWidth":0,"strokeDashArray":null,"strokeLineCap":"butt","strokeLineJoin":"miter","strokeMiterLimit":10,"scaleX":1,"scaleY":1,"angle":0,"flipX":false,"flipY":false,"opacity":1,"shadow":null,"visible":true,"clipTo":null,"backgroundColor":"","fillRule":"nonzero","globalCompositeOperation":"source-over","transformMatrix":null,"skewX":0,"skewY":0,"crossOrigin":"","alignX":"none","alignY":"none","meetOrSlice":"meet","src":"'+backgraundFPATH+'","filters":[],"resizeFilters":[]}}';
	...
	document.getElementById('moji').addEventListener("click", function(){
		...
		var fsize = Number(document.getElementById("futosaM").value);
...
//------------------------------------------------------------------------------------
	...
	document.getElementById('ENTRY').addEventListener('click', function(){
		json = JSON.stringify(canvas).slice(0, -1);													//ラストの"}"をはずす
		json += ',"otherInfo":{"SYSCODE":"'+G_SYSCODE+'","FLOORID":"'+G_FLOORID+'","REVNO":"'+G_REVNO+'"}}';	//SYSCODEとフロアIDとvarの情報を追加
		$.ajax({
			url: "update.jsp",
			type: 'POST',
			dataType: 'json',
